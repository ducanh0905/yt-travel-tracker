...
mảng du lịch: lượt xem, lượt thích, lượt bình luận, sub kênh, ngày đăng — kèm xem
bình luận từng video (có sắp xếp + dịch nhanh sang tiếng Việt).

Dữ liệu được GitHub Actions gọi API YouTube 1–2 lần/ngày, lưu thành file JSON tĩnh
...
```
channels.json                 # danh sách kênh YouTube cần theo dõi
...
public/                       # toàn bộ frontend, đây là thư mục Netlify sẽ publish
  ...
  app.js
  data/
    videos.json              # danh sách video + thống kê (được ghi tự động)
    meta.json                # thời gian cập nhật, số liệu tổng quan
    comments/<videoId>.json  # bình luận từng video (được ghi tự động)
```
...
Mở `channels.json`, điền danh sách kênh — hỗ trợ cả 3 dạng:
...
Mặc định workflow chạy **2 lần/ngày**: 23:00 UTC (~06:00 sáng giờ VN) và 11:00 UTC
(~18:00 chiều giờ VN). Muốn đổi lịch, sửa phần `cron` trong
`.github/workflows/fetch-data.yml` (cú pháp cron chuẩn UTC).
...
- Sắp xếp theo: ngày đăng, lượt xem, lượt thích, lượt bình luận, sub kênh — tăng dần/giảm dần (bấm được cả vào tiêu đề cột).
...
- Nếu 1 kênh nhập sai / không tồn tại, script sẽ ghi lỗi vào `meta.json` (mục `errors`)
  và tiếp tục xử lý các kênh còn lại, không dừng toàn bộ.
